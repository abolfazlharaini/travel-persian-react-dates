...
export function addModifier(updatedDays, day, modifier, props, state) {
  const { numberOfMonths: numberOfVisibleMonths, enableOutsideDays, orientation, momentConfig } = props;
  ...
  if (orientation === VERTICAL_SCROLLABLE) {
    ...
    currentMonth = getPreviousMonthMemoLast(currentMonth, momentConfig);
    numberOfMonths += 2;
  }
  if (!day || !isDayVisible(day, currentMonth, numberOfMonths, enableOutsideDays, momentConfig)) {
    return updatedDays;
  }

  const iso = toISODateString(day, undefined, momentConfig);
  ...
  if (enableOutsideDays) {
    ...
    const monthIso = toISOMonthString(day, undefined, momentConfig);
    const month = updatedDays[monthIso] || visibleDays[monthIso] || {};
