...
export default function getVisibleDays(
  ...
  if (!momentFactory().moment.isMoment(month)) return {};

  const visibleDaysByMonth = {};
  let currentMonth = withoutTransitionMonths ? month.clone() : month.clone().subtract(1, momentConfig.MonthProp);
  for (let i = 0; i < (withoutTransitionMonths ? numberOfMonths : numberOfMonths + 2); i += 1) {
    const visibleDays = [];

    // set utc offset to get correct dates in future (when timezone changes)
    const baseDate = currentMonth.clone();
    const firstOfMonth = baseDate.clone().startOf(momentConfig.MonthProp).hour(12);
    const lastOfMonth = baseDate.clone().endOf(momentConfig.MonthProp).hour(12);
    ...
    visibleDaysByMonth[toISOMonthString(currentMonth, undefined, momentConfig)] = visibleDays;
    currentMonth = currentMonth.clone().add(1, momentConfig.MonthProp);
  }
