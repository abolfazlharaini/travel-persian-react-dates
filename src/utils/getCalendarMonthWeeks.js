...
export default function getCalendarMonthWeeks(
  ...
  if (!momentFactory().moment.isMoment(month) || !month.isValid()) {
  ...
  const firstOfMonth = month.clone().startOf(momentConfig.MonthProp).hour(12);
  const lastOfMonth = month.clone().endOf(momentConfig.MonthProp).hour(12);
