...
class DayPicker extends React.PureComponent {
  constructor (props) {
    ...
    const currentMonth = props.hidden ? momentFactory().moment() : props.initialVisibleMonth();

    let focusedDate = currentMonth.clone().startOf(props.momentConfig.MonthProp);
  ...
  onFinalKeyDown(e) {
    ...
    switch (e.key) {
      ...
      case 'Home':
        e.preventDefault();
        newFocusedDate.startOf(momentConfig.WeekProp);
        didTransitionMonth = this.maybeTransitionPrevMonth(newFocusedDate);
        break;
      case 'PageUp':
        e.preventDefault();
  ...
  getFocusedDay(newMonth) {
    const { getFirstFocusableDay, numberOfMonths, momentConfig } = this.props;
    ...
    if (newMonth && (!focusedDate || !isDayVisible(focusedDate, newMonth, numberOfMonths, undefined, momentConfig))) {
      focusedDate = newMonth.clone().startOf(momentConfig.MonthProp);
    }
  ...
  setCalendarMonthWeeks(currentMonth) {
    const { numberOfMonths, momentConfig } = this.props;

    this.calendarMonthWeeks = [];
    let month = currentMonth.clone().subtract(1, momentConfig.MonthProp);
    const firstDayOfWeek = this.getFirstDayOfWeek();
    for (let i = 0; i < numberOfMonths + 2; i += 1) {
      const numberOfWeeks = getNumberOfCalendarMonthWeeks(month, firstDayOfWeek, momentConfig);
  ...
  updateStateAfterMonthTransition() {
    ...
    if (monthTransition === PREV_TRANSITION) {
      ...
      const numberOfWeeks = getNumberOfCalendarMonthWeeks(newInvisibleMonth, firstDayOfWeek, momentConfig);
