...
import DatePickerDataContainer from './DatePickerDataContainer';
...
const defaultProps = {
  ...
  isOutsideRange: (day) => !isInclusivelyAfterDay(day, momentFactory().moment()),
  minDate: undefined,
  maxDate: undefined,

  // internationalization
  displayFormat: () => momentFactory().moment.localeData().longDateFormat('L'),
  weekDayFormat: 'dd',
...
class DateRangePicker extends React.PureComponent {
  ...
  responsivizePickerPosition() {
    ...
    if (!withPortal && !withFullScreenPortal && this.dayPickerContainer) {
  ...
  maybeRenderDayPickerWithPortal({ visibleDays, controllerConfig }) {
    const { withPortal, withFullScreenPortal, appendToBody } = this.props;
    if (!this.isOpened() || !controllerConfig.getIsOpen()) {
      return null;
    }
    ...
    if (withPortal || withFullScreenPortal || appendToBody) {
      return (
        <Portal>
          {this.renderDayPicker({ visibleDays, controllerConfig })}
        </Portal>
      );
    }

    return this.renderDayPicker({ visibleDays, controllerConfig });
  }

  renderDayPicker({ visibleDays, controllerConfig }) {
    const {
      noNavButtons,
      rootClassName,
    ...
    const initialVisibleMonthThunk = initialVisibleMonth || (
      () => (startDate || endDate || momentFactory().moment())
    );
    ...
    return (
      ...
      >
        <DayPickerRangeController
          ...
          //   if (data)
          //     controllerConfig.onUpdateModifiersData(data);
          // }}
  ...
  render() {
    ...
    const momentConfig = config(locale);
    const input = (
      ...
      >
        <DatePickerDataContainer
          isRangePicker
          children={({ visibleDays, controllerConfig }) => this.maybeRenderDayPickerWithPortal({ visibleDays, controllerConfig })}
          datePickerProps={{
            numberOfMonths: numberOfMonths,
            orientation: orientation,
            enableOutsideDays: enableOutsideDays,
            momentConfig: momentConfig,
            focused: this.isOpened(),
