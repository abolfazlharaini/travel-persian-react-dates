...
export default class DayPickerRangeController extends React.PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (didStartDateChange) {
      ...
      //       const momentObj = momentFactory().moment(day);
  ...
  onPrevMonthClick() {
    ...
    const prevMonth = currentMonth.clone().subtract(2, momentConfig.MonthsProp);
    const prevMonthVisibleDays = getVisibleDays(prevMonth, 1, enableOutsideDays, true, momentConfig);

    const newCurrentMonth = currentMonth.clone().subtract(1, momentConfig.MonthProp);
    this.setState({
  ...
  onNextMonthClick() {
    ...
    const nextMonth = currentMonth.clone().add(numberOfMonths + 1, momentConfig.MonthProp);
    const nextMonthVisibleDays = getVisibleDays(nextMonth, 1, enableOutsideDays, true, momentConfig);
  ...
  getStateForNewMonth(nextProps, initialVisibleDays) {
    ...
    const initialVisibleMonthThunk = initialVisibleMonth || (
      startDate ? () => startDate.clone().locale(momentConfig.locale) : () => this.today
    );
  ...
  doesNotMeetMinimumNights(day) {
    ...
    if (startDate) {
      const dayDiff = day.clone().hour(12).diff(startDate.clone().startOf('day').hour(12), 'days');
      return dayDiff < minimumNights && dayDiff >= 0;
    }
    return isOutsideRange(momentFactory().moment(day).subtract(minimumNights, 'days'));
  }
  ...
  //     const dayDiff = day.clone().hour(12).diff(hoverDate.clone().startOf('day').hour(12), 'days');
  ...
  render() {
    ...
    return (
      <DayPickerFC
        ...
        selectedMonthISO={
          startDate ?
            momentConfig._toISOMonthString(startDate) :
            (endDate ? momentConfig._toISOMonthString(endDate) : null)
        }
